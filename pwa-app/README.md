...
agregar documentos, editarlos, eliminarlos, buscarlos y recibir un recordatorio de
verdad por correo (con Resend — ver **"Cómo funciona el correo con Resend"**).
...
src/
  ...
  stripeClient.js     Llama al backend de pagos (Netlify Functions o /server) y redirige a Stripe
  emailClient.js       Llama al backend de correo (Netlify Functions o /server) para mandar con Resend
  ...
  adminClient.js        Llama a /api/admin-metrics para el panel de administrador (ver app.js)
...
netlify/functions/    Backend de pagos, correo, borrado de cuenta y panel de administrador
                       como funciones de Netlify — ver "Cómo funciona el pago con Stripe",
                       "Cómo funciona el correo con Resend", "Cómo funciona el panel de
                       administrador" y "Desplegar en Netlify"
supabase/
...
../server/            El mismo backend de pagos y correo, como servidor
...
- **Cuentas**: pantalla de bienvenida con "Crear cuenta" / "Iniciar sesión" (nombre,
  ...
  registrarse. Cada cuenta ve únicamente sus propios documentos — la sesión persiste
  entre visitas (no hay que volver a iniciar sesión cada vez que se abre la app) y
  "Cerrar sesión" está en Perfil. Ver **"Cómo funcionan las cuentas hoy"** más abajo.
...
- Agregar/editar documento: foto o PDF real (se guarda como miniatura), categoría,
  fecha de vencimiento con atajos, recordatorio configurable, notas.
- Ocho categorías: **Personal** (documentos de identificación: INE, pasaporte,
  ...
  (colegiaturas e inscripciones), **Pagos** (recibos recurrentes) y
  **Préstamos** (dinero prestado o debido), además de Salud.
  - **Vehículo** y **Hogar** aceptan dos tipos de registro en la misma
    categoría: un documento normal (con vencimiento) o un registro de
    **Mantenimiento** (bitácora de cuándo se hizo algo — servicio del auto,
    presión de llantas, aire acondicionado, cableado eléctrico). Al abrir la
    categoría se ven ambos tipos juntos, ordenados por urgencia, con chips
    Todos/Documentos/Mantenimiento para filtrar. El formulario cambia según
    el tipo elegido: un documento pide "fecha de vencimiento"; un
    mantenimiento pregunta "¿cuándo lo hiciste?" y deja programar un
    recordatorio opcional para la próxima vez.
...
- Recordatorios: agrupados por urgencia (vencidos, esta semana, este mes, más
  adelante) + botón para mandarte un recordatorio por correo de verdad (ver
  **"Cómo funciona el correo con Resend"** más abajo).
...
- **Planes de tres niveles, con pago real por Stripe (modo prueba)** — Perfil
  muestra el plan actual, cuántos elementos se han guardado sobre el límite
  del plan (Salud no cuenta para este límite) y una tarjeta para ver/cambiar
  de plan:
  - **Gratis**: hasta 10 elementos (fuera de Salud) + Vacunas sin límite. No
    pide tarjeta ni pasa por Stripe — bajar a este plan es instantáneo.
  ...
  **el plan solo se activa cuando el navegador vuelve y el propio backend
  confirma con Stripe que de verdad hubo un cobro (o que empezó la
  prueba)** — nunca al hacer clic en el botón (ver **"Cómo funciona el pago
  con Stripe"** más abajo, donde también está la explicación de por qué esto
  necesita un servidor y cómo correrlo). Intentar guardar un elemento fuera
  ...
  La demo interactiva de una sola página (`artifact-demo/memoria.html`, sin
  servidor) no puede hacer esto — un archivo estático no puede tener una
  llave secreta de Stripe ni recibir sus webhooks — así que ahí se mantiene
  el pago simulado de antes, con una nota en pantalla que lo aclara.
...
- **Facturación**: Perfil → "Facturación" es el registro de los pagos de tu
  propia membresía de Memoreo (no un lugar para guardar tus propias
  facturas de compra). Si ya tuviste un plan de paga de verdad, va directo
  al portal de facturación de Stripe — ahí es donde de verdad viven tus
  facturas, no una copia dentro de la app; si nunca has tenido un plan de
  paga, explica que todavía no hay nada que mostrar y ofrece "Ver planes".
...
## Cómo funciona el correo con Resend

El botón "Probar recordatorio por correo" en Recordatorios manda un
**correo real** con [Resend](https://resend.com), igual de real que los
cargos de Stripe: pasa por un backend (las mismas dos opciones que Stripe —
funciones de Netlify o `/server`) que es el único que tiene la API key. No
hay ningún interruptor de "activar/desactivar" delante — el botón manda el
correo directamente, sin un paso simulado antes.

### Configurarlo

1. Crea una cuenta gratis en [resend.com](https://resend.com).
2. En **API Keys**, crea una llave nueva (`re_...`).
3. Agrégala como `RESEND_API_KEY` — en `pwa-app/.env` si usas `netlify dev`,
   o en `server/.env` si usas el servidor Express (ver **"Correrlo en
   local"** en la sección de Stripe más arriba — el mismo archivo `.env`
   sirve para las dos cosas).

Con eso ya funciona — no hace falta configurar nada más para probarlo.

### La restricción que hay que conocer

Sin verificar tu propio dominio en Resend, **solo puedes mandarte correos a
ti mismo**: a la dirección con la que te registraste en Resend, no a
cualquier destinatario — es una medida anti-abuso del propio Resend para
cuentas nuevas, no algo de este proyecto. Como el botón de prueba manda el
correo a `state.account.email` (el correo con el que iniciaste sesión en
Memoreo), esto significa que **para que la prueba llegue de verdad, tienes
que iniciar sesión en Memoreo con el mismo correo con el que te registraste
en Resend** — con la cuenta demo (`demo@memoreo.com`) nunca va a llegar, a
menos que esa sea literalmente tu cuenta de Resend.

Si envías a cualquier otra dirección sin haber verificado un dominio,
Resend rechaza el envío y la app te muestra un mensaje explicándolo en vez
de un error genérico (ver `netlify/functions/send-reminder-email.mjs` o
`server/index.js`). Para mandar a cualquier persona de verdad — lo que
haría falta en producción — verifica un dominio propio en Resend
(**Domains → Add Domain**, agregar unos registros DNS) y cambia
`RESEND_FROM` por algo como `Memoreo <recordatorios@tudominio.com>`.
...
## Cómo funciona el pago con Stripe (modo prueba)
...
1. Elige un usuario y una contraseña, y agrégalos como `ADMIN_USERNAME` y
   ...
   **"2. Configurar Stripe, Resend y el panel de administrador en
   Netlify"** abajo, donde también van estas).
...
Sin `ADMIN_USERNAME` y `ADMIN_PASSWORD` configuradas, `/api/admin-metrics`
responde con un error claro en vez de dejar entrar a cualquiera.

### La cortina de seguridad, honestamente

Esto es un usuario y una contraseña simples comparados del lado del
servidor (`netlify/functions/admin-metrics.mjs`, con una comparación en
tiempo constante para no filtrar por cuánto tarda la respuesta si alguien
está adivinando), no un sistema de autenticación completo: es un solo
usuario-administrador fijo (el que tú definas en las variables de entorno),
...
- [ ] `RESEND_API_KEY` configurada para que el recordatorio por correo
      salga de verdad (ver **"Cómo funciona el correo con Resend"**).
...
pagos activan planes reales vía Stripe, el recordatorio por correo se manda
de verdad con Resend, y el panel de administrador muestra cuentas reales.
...
### 2. Configurar Stripe, Resend y el panel de administrador en Netlify
...
1. En el dashboard del sitio: **Project configuration → Environment
   ...
   `STRIPE_PRICE_PREMIUM`, `STRIPE_PRICE_PREMIUM_PLUS`, `RESEND_API_KEY`,
   `ADMIN_USERNAME` y `ADMIN_PASSWORD` (ver **"Cómo funciona el panel de
   administrador"** más arriba) y, si ya verificaste un dominio propio en
   Resend, `RESEND_FROM`.
...
ya en una URL que puedes compartir. El botón de "Probar recordatorio por
correo" en Avisos manda un correo real de la misma forma — recuerda la
restricción de Resend (ver **"Cómo funciona el correo con Resend"**): sin
dominio propio verificado, solo llega si inicias sesión con el mismo correo
...
2. En tu proyecto, abre **SQL Editor**, pega el contenido completo de
   ...
   `profiles` y `documents`, activa Row Level Security con las políticas de
   "cada quien ve y edita solo lo suyo", y crea los dos buckets privados de
   Storage (`attachments` y `avatars`) con sus propias políticas de acceso.
   Es seguro volver a correrlo si algo falla a medias.
...
4. Agrega las cuatro variables (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`,
   ...
   en Netlify para producción (ver **"2. Configurar Stripe, Resend y el
...
- **`public.profiles`** — un renglón por cuenta (nombre, plan, color,
  método de pago, datos de Stripe, preferencias de notificación), ligado
  1 a 1 a `auth.users` por el mismo id. Un trigger de la base de datos
  (`handle_new_user`) crea este renglón solo, en el mismo instante en que
  alguien se registra — nada en el código de la app tiene que acordarse de
  crearlo.
...
El envío en sí ya es real — el botón de correo manda con Resend de verdad
(ver **"Cómo funciona el correo con Resend"**) y las notificaciones usan la
API real del navegador — pero ambos son "bajo demanda" (hay que tener la
app abierta y tocar el botón). Para que lleguen automáticamente el día que
corresponde sin que nadie abra la app, hace falta una función programada
del lado del servidor (un Netlify Scheduled Function, por ejemplo) que
consulte `public.documents` por vencimientos próximos y mande el correo o
la notificación push sola — con la base de datos real ya en su lugar, esto
es agregar esa función; ya no falta nada más para que sea posible.
