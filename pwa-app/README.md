...
- Ocho categorías: **Personal** (documentos de identificación: INE, pasaporte,
  ...
  **Préstamos** (dinero prestado o debido), además de Salud.
  - **Vehículo** y **Hogar** aceptan dos tipos de registro en la misma
    categoría, con chips Todos/Documentos (o Servicios, en Hogar)/Mantenimiento
    para filtrar:
    - En **Vehículo**, el primer tipo es un documento normal (con
      vencimiento) — tarjeta de circulación, póliza, verificación.
    - En **Hogar**, ese mismo primer tipo se llama **Servicios** (agua, luz,
      gas, internet) y funciona igual que la categoría **Pagos**: se
      registra **una sola vez** con la próxima fecha de pago conocida y una
      frecuencia (mensual, bimestral o anual), con monto opcional, y
      Memoreo calcula sola la siguiente fecha cada vez que la anterior pasa
      — nunca hay que volver a capturar el pago de agua o luz cada mes o
      cada bimestre.
    - En ambas, **Mantenimiento** es una bitácora de cuándo se hizo algo
      (servicio del auto, presión de llantas, aire acondicionado, cableado
      eléctrico): pregunta "¿cuándo lo hiciste?" y deja programar un
...
- **Planes de tres niveles, con pago manual (transferencia bancaria MX o
  ...
  - **Gratis**: hasta 10 elementos (fuera de Salud) + Vacunas sin límite. Bajar
...
- **Facturación**: Perfil → "Facturación" es el registro de los pagos de tu
  ...
  facturas de compra) — muestra el plan activo y hasta cuándo vale, según la
  fecha que le puso el administrador al activar tu última solicitud de pago.
...
3. **Activar**, una vez confirmado el pago: el administrador toca "Activar"
   en su panel. Eso llama a `netlify/functions/admin-payment-requests.mjs`
   (acción `activate`), que sube `profiles.plan` al plan pedido y le pone
   `profiles.plan_expires_at` a hoy + los meses pagados — o, si la cuenta ya
   tenía un plan de paga vigente, los meses se **suman** a partir de esa
   fecha en vez de reiniciar desde hoy, para que pagar por adelantado nunca
   haga perder tiempo ya pagado. La solicitud queda marcada `activado`, con
   cuándo y qué administrador la activó. "Cancelar" en cambio marca
   `cancelado` sin tocar el plan — para cuando el pago nunca se completó.
4. **Vencimiento automático**: `maybeExpirePlan` (`src/auth.js`) se llama
   cada vez que se recupera la sesión o se inicia sesión — si
   `plan_expires_at` ya pasó, la cuenta baja sola a Gratis (y limpia
   `plan_expires_at`), sin que nadie tenga que acordarse de hacerlo a mano.

`payment_requests` (esquema completo en **"Base de datos real (Supabase)"**)
es una tabla nueva, separada de `profiles` — conserva un historial completo
de todo lo pedido, pagado o cancelado, nunca se borra un renglón.
...
tienen botones **Activar** (sube el plan de esa cuenta y le pone fecha de
vencimiento — solo después de confirmar que el pago de verdad llegó) y
**Cancelar** (si nunca se completó). Todo esto lo maneja
`netlify/functions/admin-payment-requests.mjs`, con la misma llave
service_role y la misma cortina de usuario/contraseña que `admin-metrics.mjs`.
...
2. En tu proyecto, abre **SQL Editor**, pega el contenido completo de
   ...
   `profiles`, `documents` y `payment_requests`, activa Row Level Security
   con las políticas de "cada quien ve y edita solo lo suyo", y crea los dos
   buckets privados de Storage (`attachments` y `avatars`) con sus propias
   políticas de acceso. Es seguro volver a correrlo si algo falla a medias —
   incluso si ya tenías un proyecto viejo sin `payment_requests` ni
...
- **`public.payment_requests`** — un renglón por solicitud de pago manual
  ...
  (`pendiente`/`activado`/`cancelado`). Cada quien puede insertar y ver las
  suyas (RLS), pero solo el panel de administrador (con la llave
  service_role, que ignora RLS) puede cambiar el estado — así nadie puede
  activarse un plan a sí mismo.
