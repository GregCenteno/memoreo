...
3. **Activar**, una vez confirmado el pago: el administrador toca "Activar"
   en su panel — ahí mismo ve hasta cuándo va a quedar vigente el plan antes
   de confirmar. Eso llama a `netlify/functions/admin-payment-requests.mjs`
   ...
   cuándo, qué administrador la activó y hasta cuándo quedó vigente
   (columna "Vencimiento" en el panel). "Cancelar" en cambio marca
...
4. **Revertir**, si "Activar" se tocó por error: mientras la solicitud siga
   ...
   cuándo se revirtió (`reverted_at`), para que quede el rastro.
5. **Vencimiento automático**: `maybeExpirePlan` (`src/auth.js`) se llama
...
de todo lo pedido, pagado, cancelado o revertido, nunca se borra un renglón.
...
**Cancelar** (si nunca se completó). Una vez activada, la columna
"Vencimiento" muestra hasta cuándo queda vigente ese plan, y el botón
cambia a **Revertir** por si "Activar" se tocó por error — deshace
exactamente esa activación (el plan y el vencimiento vuelven a como estaban
antes) y la solicitud regresa a "Pendiente" para decidir de nuevo. Todo
esto lo maneja `netlify/functions/admin-payment-requests.mjs`, con la misma
llave service_role y la misma cortina de usuario/contraseña que
`admin-metrics.mjs`.
