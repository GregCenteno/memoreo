...
// A "Salud" entry is one of cuatro subtipos, cada uno con su propia forma de
// hablar de su fecha en vez de un "vence" genérico: a vacuna's date is its
// next dose, a medicamento's is when it runs out (so you know when to buy
// more — not an expiration in the document sense), a laboratorio's is its
// next follow-up appointment, and a receta's is simply when it deja de ser
// válida (vencimiento normal, como cualquier documento). All fall back to
// "cuándo se registró" when there's no forward-looking date yet.
export function healthInfo(doc, today = new Date()) {
  if (doc.healthType === 'receta') {
    if (doc.expiresAt) return daysInfo(doc, today);
    if (doc.performedAt) return { days: null, urgency: 'none', label: `Expedida el ${fmtDate(doc.performedAt)}` };
