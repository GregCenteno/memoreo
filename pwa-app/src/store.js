...
export const CATS = [
  ...
  // una frecuencia, y Memoreo calcula sola la siguiente fecha de pago.
  { id: 'pagos', label: 'Pagos', icon: 'repeat', noun: 'pago' },
...
// Las 3 subcategorías del módulo de Salud. Cada plan desbloquea un subconjunto
...
// laboratorio; Premium Plus no desbloquea una subcategoría nueva, sino una
...
export const PLANS = {
  gratis: { id: 'gratis', label: 'Plan Gratis', price: '$0', limit: 10, salud: ['vacuna'], advancedHealth: false, colorPersonalization: false },
  premium: { id: 'premium', label: 'Plan Premium', price: 'desde $59/mes', limit: 20, salud: ['vacuna', 'medicamento', 'laboratorio'], advancedHealth: false, colorPersonalization: false },
  premium_plus: { id: 'premium_plus', label: 'Plan Premium Plus', price: 'desde $99/mes', limit: Infinity, salud: ['vacuna', 'medicamento', 'laboratorio'], advancedHealth: true, colorPersonalization: true }
};
...
export function docNoun(doc) {
  if (doc.category === 'pagos') return 'pago';
...
export function seedData() {
  ...
  return [
    ...
    { id: 6, name: 'Contrato de Renta', category: 'hogar', kind: 'doc', expiresAt: addDays(today, 146).toISOString(), reminderDays: 30, notes: 'Depósito: 1 mes. Contacto casero: Sra. Martínez.', image: null, performedAt: null },
    ...
    // Salud: Vacunas viene incluida en el plan Gratis desde el primer uso, y
    // la cuenta demo arranca en Premium (ver startDemoSession en auth.js), así
    // que también se ve Medicamentos y dos Pruebas de laboratorio del mismo
    // tipo con fechas distintas — para que la gráfica de tendencia (Premium
    // Plus) tenga algo real que mostrar en cuanto se actualiza el plan.
...
// Siembra la cuenta demo con los 24 documentos de ejemplo (seedData()) la
// primera vez que se crea — ver startDemoSession() en auth.js, que solo
// llama a esto con seeded=true cuando la cuenta demo apenas se registró. Una
// cuenta nueva de verdad (seeded=false) no necesita hacer nada: en Postgres
// "no tener documentos todavía" ya es el estado natural de una cuenta
// recién creada, a diferencia de localStorage donde había que inicializar
// la llave a mano.
