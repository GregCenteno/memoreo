...
// La fecha concreta que "Marcar como atendido" (ver isHandled() más abajo)
// necesita como referencia: para un gasto recurrente (Pagos, o Servicios
// dentro de Hogar) es la próxima fecha de pago que calcula recurrenceInfo()
// —la misma que ya se le muestra a la persona—; para cualquier otra cosa
// (documentos, mantenimiento, salud, préstamos) es simplemente su
...
// doc.attendedUntil (fecha que guardó markAttended en app.js, ver
// Store.setAttended más abajo) alcanza o pasa la fecha vigente que
// calcula dueDateFor(). Por eso no hace falta desmarcarlo a mano cuando
// vuelve a tocar pagar: en cuanto esa fecha avanza (el ciclo siguiente de
// un pago recurrente) o cambia (se editó el vencimiento de un documento),
// deja de cumplirse la comparación y el elemento reaparece solo en "Vence
// pronto" — ver homeView() en views.js.
export function isHandled(doc, today = new Date()) {
  if (!doc.attendedUntil) return false;
  const due = dueDateFor(doc, today);
  if (!due) return false;
  return due.getTime() <= new Date(doc.attendedUntil).getTime();
}
...
function rowToDoc(row, signedUrl) {
  return {
    ...
    // row.kind ya viene como uno de los cuatro valores válidos ('doc',
    // 'activity', 'documento' o 'pago' — ver la restricción check en
    // supabase/schema.sql), así que basta con pasarlo tal cual para las
    // categorías que de verdad lo usan (Vehículo/Hogar); en cualquier otra
    // categoría se guarda 'doc' por default en la base de datos pero no
    // significa nada ahí, así que se deja en null como antes. Antes esto
    // colapsaba cualquier valor que no fuera 'activity' a 'doc' sin
    // importar cuál fuera realmente el guardado — perdía la distinción de
    // Documentos/Servicios de Hogar en cuanto se recargaba la app (volvía
    // a pedir la lista a Supabase), aunque se hubiera guardado bien.
    kind: catInfo(row.category).activityCapable ? row.kind : null,
    expiresAt: row.expires_at,
    performedAt: row.performed_at,
    attendedUntil: row.attended_until || null,
...
class Store {
  ...
  // Returns documents with expiresAt/performedAt/attendedUntil as real Date objects for rendering.
  all() {
    return this.docs.map((d) => ({
      ...d,
      expiresAt: d.expiresAt ? new Date(d.expiresAt) : null,
      performedAt: d.performedAt ? new Date(d.performedAt) : null,
      attendedUntil: d.attendedUntil ? new Date(d.attendedUntil) : null
    }));
  }
  ...
  // Marca (due = fecha vigente, ver dueDateFor()) o desmarca (due = null)
  // esta entrada como "atendida" — la acción rápida de "Marcar como
  // atendido"/"Deshacer" en la tarjeta de "Vence pronto" y en el detalle
  // (ver markAttended()/unmarkAttended() en src/app.js). No pasa por
  // payloadToRow()/update() porque no es un campo del formulario de
