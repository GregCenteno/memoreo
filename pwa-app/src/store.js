...
export const PLANS = {
  ...
  premium_plus: { id: 'premium_plus', label: 'Plan Premium Plus', price: 'desde $99/mes', limit: Infinity, salud: ['vacuna', 'medicamento', 'laboratorio', 'receta'], advancedHealth: true, colorPersonalization: true }
};
...
export function formMode(category, kind) {
  ...
  // La subcategoría "Servicios" de Hogar (agua, luz, gas, internet…) es,
  // en el fondo, el mismo tipo de gasto recurrente que la categoría Pagos
  // — mismo nombre, misma idea — así que usa exactamente su mismo
  // formulario (frecuencia + próxima fecha + monto opcional) en vez del de
  // documento normal con vencimiento. Mantenimiento (kind === 'activity',
  // ya filtrado arriba) sigue siendo su propia bitácora, sin cambios.
  if (category === 'hogar') return 'pago';
  return 'doc';
}
...
export function docNoun(doc) {
  ...
  if (doc.category === 'hogar' && !isActivity(doc)) return 'servicio';
...
export function docInfo(doc) {
  ...
  if (doc.category === 'hogar' && !isActivity(doc)) return recurrenceInfo(doc);
