...
import {
  ...
  LAB_TEST_TYPES, labTestTypeInfo, advancedHealthAllowed, colorPersonalizationAllowed
} from './store.js';
...
export function categoryView(catId, docs, kindFilter, plan) {
  ...
  if (showHealthFilter) {
    ...
    html += `<div class="chip-row" style="margin-bottom:16px;">
      <button class="chip ${!kindFilter ? 'selected' : ''}" data-kind-filter="">Todos</button>
      <button class="chip ${kindFilter === 'doc' ? 'selected' : ''}" data-kind-filter="doc">${icon('doc')} Documentos</button>
...
export function remindersView(docs) {
  ...
  const mostUrgent = withDates.slice().sort((a, b) => docInfo(a).days - docInfo(b).days)[0];
  if (mostUrgent) {
    html += `<button class="btn btn-outline" style="margin:14px 0 4px;" data-action="test-email-reminder">${icon('mail')} Probar recordatorio por correo</button>`;
  }
...
export function addView({ draft, editing, plan }) {
  ...
  const noun = mode === 'pago' ? 'pago' : mode === 'prestamo' ? 'préstamo' : mode === 'activity' ? 'registro'
  ...
  if (ci && ci.activityCapable) {
    html += `<div class="field"><label>Tipo de registro</label><div class="chip-row" id="kindChips">
      <button type="button" class="chip ${mode !== 'activity' ? 'selected' : ''}" data-set-kind="doc">${icon('doc')} Documento</button>
  ...
  if (mode === 'salud') {
    ...
    if (draft.healthType === 'medicamento') {
      ...
      const lt = labTestTypeInfo(draft.labTestType);
      html += `<div class="field"><label>Tipo de prueba</label><div class="chip-row" id="labTypeChips">${LAB_TEST_TYPES.map((t) =>
        `<button type="button" class="chip ${draft.labTestType === t.id ? 'selected' : ''}" data-set-lab-type="${t.id}">${esc(t.label)}</button>`
      ).join('')}</div></div>`;
      ...
      html += `<div class="field"><label>Resultados (opcional)</label><div class="lab-fields-grid" id="labFields">${lt.fields.map((f) =>
        `<div class="lab-field"><label for="fLab_${f.key}">${esc(f.label)}${f.unit ? ` (${esc(f.unit)})` : ''}</label><input type="number" step="any" inputmode="decimal" id="fLab_${f.key}" data-lab-field="${f.key}" placeholder="0" value="${esc((draft.labValues && draft.labValues[f.key]) || '')}" /></div>`
      ).join('')}</div></div>`;
...
export function detailView(doc, { plan } = {}) {
  ...
  if (ci.activityCapable) {
    html += `<div class="info-row"><span class="k">Tipo</span><span class="v wrap">${activity ? 'Mantenimiento' : 'Documento'}</span></div>`;
  }
  if (isPago) {
    ...
    if (doc.healthType === 'medicamento') {
      ...
      const lt = labTestTypeInfo(doc.labTestType);
      html += `<div class="info-row"><span class="k">Prueba</span><span class="v wrap">${esc(lt.label)}</span></div>
      <div class="info-row"><span class="k">Fecha de la prueba</span><span class="v">${doc.performedAt ? fmtDate(doc.performedAt) : 'Sin registrar'}</span></div>`;
      lt.fields.forEach((f) => {
        const val = doc.labValues && doc.labValues[f.key];
        html += `<div class="info-row"><span class="k">${esc(f.label)}</span><span class="v">${val ? `${esc(val)}${f.unit ? ' ' + esc(f.unit) : ''}` : '—'}</span></div>`;
      });
      html += `<div class="info-row"><span class="k">Próxima cita médica</span><span class="v">${doc.expiresAt ? fmtDate(doc.expiresAt) : 'Sin fecha'}</span></div>`;
    } else {
