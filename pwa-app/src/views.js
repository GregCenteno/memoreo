...
export function paymentRequestSheet(planId, account) {
  ...
  // "Pagar vía WhatsApp": un camino más directo que la solicitud de arriba
  // — abre WhatsApp con el número del negocio (VITE_WHATSAPP_NUMBER, ver
  // store.js) y un mensaje ya escrito diciendo qué plan quiere. Disponible
  // desde que se elige el plan, sin esperar a elegir duración/método — el
  // texto del mensaje se ajusta solo con lo que ya se haya elegido (ver
  // updateWhatsappLink en openPaymentRequest, app.js). Si no hay número
  // configurado, WHATSAPP_NUMBER queda vacío y este botón no se dibuja.
  const whatsappBtn = WHATSAPP_NUMBER
    ? `<a class="btn btn-outline" id="prWhatsappBtn" href="https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(whatsappPayMessage(planId, null, null))}" target="_blank" rel="noopener" style="display:flex;align-items:center;justify-content:center;gap:8px;">${icon('whatsapp')} Pagar vía WhatsApp</a>
      <p style="text-align:center;color:var(--ink-faint);font-size:0.78rem;margin:12px 0;">o deja tu solicitud y te contactamos nosotros</p>`
    : '';
