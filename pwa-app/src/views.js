...
// Etiqueta del filtro/tipo "Documento" para categorías con más de un tipo de
// registro. Vehículo solo tiene Documentos/Mantenimiento y usa esto tal
// cual; Hogar tiene un tercer tipo (Servicios) además de Documentos, así
// que sus tres chips se arman aparte en cada lugar que los usa (ver
// categoryView, addView y detailView más abajo) en vez de pasar por aquí.
function docKindLabel() {
  return 'Documentos';
}
function docKindLabelSingular() {
  return 'Documento';
}
...
export function urgentCard(doc) {
  const info = docInfo(doc);
  // El botón de "atendido" (ver markAttended() en app.js) va aparte del
  // resto de la tarjeta, que sigue abriendo el detalle al tocarla — su
  // propio manejador de clic (data-attend) se revisa antes que el de
  // data-open en handleClick(), así que tocarlo no navega también.
  return `<div class="urgent-card" data-open="${doc.id}">
    <div class="urgent-card-top">
      ${badge(info)}
      <button type="button" class="urgent-attend-btn" data-attend="${doc.id}" title="Marcar como atendido" aria-label="Marcar como atendido">${icon('check')}</button>
    </div>
    <div class="name">${esc(doc.name)}</div>
...
// Pantalla de "tu suscripción venció" — reemplaza TODA la app (sin tabbar,
// sin acceso a inicio/documentos/perfil) mientras
// account.subscriptionExpired sea cierto (ver buildAccount en auth.js y
// render() en app.js, que la fuerza sin importar a qué ruta se intentara
// navegar). No es un candado total sin salida: "Renovar suscripción" abre
// la misma hoja de solicitud de pago de siempre, y "Continuar con Plan
// Gratis" es la salida instantánea y sin costo de siempre (setPlan) — la
// idea es que nadie siga usando un plan de paga vencido sin darse cuenta,
// no dejar a alguien con su cuenta secuestrada si ya no quiere pagar.
export function renewalRequiredView(account) {
  ...
  <div class="renewal-wrap">
    <div class="renewal-icon">${icon('lock')}</div>
    <h1>Tu suscripción venció</h1>
    <p class="auth-lead">Tu ${esc(plan.label)}${expiresAt ? ` venció el ${esc(fmtDate(expiresAt))}` : ' ya venció'}. Renueva tu suscripción para seguir usando Memoreo — tus documentos siguen guardados tal cual y los recuperas en cuanto se active de nuevo tu plan.</p>
    <div class="renewal-actions">
      <button class="btn btn-primary" data-action="open-plans">Renovar suscripción</button>
      <button class="btn btn-ghost" data-action="refresh-account">${icon('repeat')} Ya pagué — actualizar</button>
    </div>
    <p class="renewal-alt">¿No quieres renovar? <button type="button" class="link-inline" data-action="continue-gratis">Continuar con Plan Gratis</button></p>
    <button type="button" class="link-inline renewal-logout" data-action="logout">Cerrar sesión</button>
  </div>`;
}
...
export function authView({ mode, error }) {
  ...
  if (!isSignup) {
    html += `<p class="auth-switch" style="margin-top:-2px;margin-bottom:4px;"><button type="button" class="link-inline" id="forgotPasswordBtn">¿Olvidaste tu contraseña?</button></p>`;
  }
  // Prendido por defecto — es como se comportaba Memoreo antes de que
  // existiera esta opción (sesión guardada en localStorage, dura entre
  // visitas). Apagarlo hace que la sesión se guarde solo mientras esa
  // pestaña siga abierta (ver setRememberSession en supabaseClient.js) — útil
  // en una computadora compartida o un cibercafé.
  html += `<div class="field" style="margin-top:-6px;"><div class="toggle-row"><span class="t">Mantener la sesión iniciada</span><div class="switch on" id="authRememberSwitch"></div></div>
    <p class="preset-hint" style="margin-top:6px;">Si lo apagas, se cierra sola en cuanto cierres el navegador — útil en una computadora que no es tuya.</p></div>`;
...
// Pantalla de "Nueva contraseña" (state.route === 'recovery', ver
// parseRecoveryHash/postRenderNewPassword en app.js) — a la que se llega
// solo desde la liga de un solo uso que manda requestPasswordReset(). A
// propósito el topbar NO trae botón de "atrás": salir a medias dejaría
// colgada la sesión temporal de recuperación (ver beginPasswordRecovery en
// auth.js) sin que la persona metiera una contraseña nueva de verdad — el
// único camino de salida es #recoveryCancelBtn, que sí cierra esa sesión
// antes de volver a la pantalla de bienvenida.
export function newPasswordView({ error }) {
  let html = `<div class="topbar"><h1>Nueva contraseña</h1></div>`;
  html += `<div class="auth-wrap">`;
  html += `<p class="auth-lead">Escoge una contraseña nueva para tu cuenta.</p>`;
  html += `<div class="field"><label for="newPassword1">Contraseña nueva</label>
    <div class="password-field">
      <input type="password" id="newPassword1" placeholder="Mínimo 6 caracteres" autocomplete="new-password" />
      <button type="button" class="password-toggle" id="newPasswordToggle" aria-label="Mostrar contraseña" aria-pressed="false" tabindex="-1">${icon('eye')}</button>
    </div>
  </div>`;
  html += `<div class="field"><label for="newPassword2">Confirmar contraseña nueva</label><input type="password" id="newPassword2" placeholder="Repite la contraseña" autocomplete="new-password" /></div>`;
  if (error) html += `<p class="auth-error">${esc(error)}</p>`;
  html += `<button class="btn btn-primary" id="newPasswordSubmitBtn" disabled>Guardar contraseña</button>`;
  html += `<p class="auth-switch"><button type="button" class="link-inline" id="recoveryCancelBtn">Cancelar e iniciar sesión más tarde</button></p>`;
  html += `</div>`;
  return html;
}

export function homeView({ docs, installDismissed, canInstall, account }) {
  // isHandled() excluye lo que ya se marcó como "atendido" para su
  // vencimiento/pago actual (ver Marcar como atendido en urgentCard()/
  // detailView(), y dueDateFor()/isHandled() en store.js) — vuelve a
  // aparecer aquí solo hasta que de verdad toque el siguiente ciclo.
  const urgent = docs
    .filter((d) => ['crit', 'warn'].includes(docInfo(d).urgency) && !isHandled(d))
...
export function categoryView(catId, docs, kindFilter, plan) {
  ...
  if (showKindFilter && kindFilter) {
    // Hogar filtra en tres grupos (Documentos/Servicios/Mantenimiento);
    // Vehículo (el otro activityCapable) solo tiene dos (Documentos/
    // Mantenimiento, filtro "doc") — ver el chip-row más abajo.
    if (kindFilter === 'activity') list = list.filter((d) => isActivity(d));
    else if (catId === 'hogar' && kindFilter === 'documento') list = list.filter((d) => !isActivity(d) && d.kind === 'documento');
    else if (catId === 'hogar' && kindFilter === 'pago') list = list.filter((d) => !isActivity(d) && d.kind !== 'documento');
    else list = list.filter((d) => !isActivity(d));
  }
  ...
  if (showHealthFilter) {
  ...
  } else if (showKindFilter && catId === 'hogar') {
    html += `<div class="chip-row" style="margin-bottom:16px;">
      <button class="chip ${!kindFilter ? 'selected' : ''}" data-kind-filter="">Todos</button>
      <button class="chip ${kindFilter === 'documento' ? 'selected' : ''}" data-kind-filter="documento">${icon('doc')} Documentos</button>
      <button class="chip ${kindFilter === 'pago' ? 'selected' : ''}" data-kind-filter="pago">${icon('repeat')} Servicios</button>
    ...
    html += `<div class="chip-row" style="margin-bottom:16px;">
      <button class="chip ${!kindFilter ? 'selected' : ''}" data-kind-filter="">Todos</button>
      <button class="chip ${kindFilter === 'doc' ? 'selected' : ''}" data-kind-filter="doc">${icon('doc')} ${docKindLabel()}</button>
      <button class="chip ${kindFilter === 'activity' ? 'selected' : ''}" data-kind-filter="activity">${icon('wrench')} Mantenimiento</button>
    </div>`;
  }
  if (!list.length) {
    const emptyCopy = showKindFilter && kindFilter === 'activity'
      ...
      : showKindFilter && kindFilter === 'pago'
      ? 'Aún no tienes servicios guardados aquí.<br/>Toca + para agregar el primero.'
      : showKindFilter && (kindFilter === 'doc' || kindFilter === 'documento')
...
// `history`: los pagos de esta cuenta que ya se activaron (status
// 'activado' en public.payment_requests — ver paymentHistory() en
// src/auth.js, y openSubscription() en app.js, que la carga antes de abrir
// esta hoja). Ordenados del más reciente al más viejo; puede venir vacío
// (cuenta que nunca ha pagado, o mientras se está cargando — ver el estado
// intermedio en openSubscription()).
export function subscriptionSheet(account, history) {
  const plan = planInfo(account && account.plan);
  const expiresAt = account && account.planExpiresAt ? new Date(account.planExpiresAt) : null;
  ...
  const intro = rows.length
    ...
    : `<p>Estás en el Plan Gratis. Los planes de paga (Premium y Premium Plus) se activan por transferencia bancaria (México) o PayPal: eliges cuántos meses y nos escribes por WhatsApp, y en cuanto se confirma el pago, activamos tu plan.</p>`;

  // Historial: un renglón por cada vez que se activó un pago — "1 mes de
  // Premium en agosto", "6 meses de Premium Plus en septiembre", etc., con
  // el método y el monto pagado (meses × precio mensual del plan de ESE
  // pago, no del plan actual — así un pago viejo de Premium se sigue
  // mostrando con el precio de Premium aunque hoy la cuenta ya sea Premium
  // Plus). No incluye solicitudes 'pendiente' ni 'cancelado': esto es un
  // historial de lo que sí se pagó, no de lo que se pidió.
  const historyList = (history && history.length)
    ? `<div class="section-head" style="margin-top:22px;"><h2 style="font-size:0.95rem;">Historial de pagos</h2></div>
      <div class="info-card">${history.map((h) => {
        const hp = planInfo(h.plan);
        const monthsLabel = h.months === 1 ? '1 mes' : `${h.months} meses`;
        const method = PAYMENT_METHODS.find((m) => m.id === h.paymentMethod);
        const when = h.activatedAt ? `Activado el ${fmtDate(new Date(h.activatedAt))}` : 'Fecha no registrada';
        const amount = hp.monthlyPrice ? formatMoney(hp.monthlyPrice * (h.months || 0)) : '';
        return `<div class="info-row"><span class="k" style="display:flex;align-items:center;gap:8px;">${icon('cash')} <span>${esc(hp.label)} · ${esc(monthsLabel)}<br><span style="font-weight:400;color:var(--ink-faint);">${esc(when)}${method ? ` · ${esc(method.label)}` : ''}</span></span></span><span class="v wrap">${esc(amount)}</span></div>`;
      }).join('')}</div>`
    : '';
  ...
  return `<div class="sheet-handle"></div><h3>${icon('receipt')} Mi suscripción</h3>
    ...
    ${list}
    ${historyList}
    <button class="btn btn-primary" data-action="open-plans" style="margin-top:${historyList ? '18px' : '0'};">Ver planes</button>`;
}
...
export function plansSheet(account) {
  ...
  // Un plan vencido (ver subscriptionExpired en auth.js) NO cuenta como
  // "tu plan actual" aquí — si contara, su botón saldría deshabilitado
  // ("Tu plan actual") justo cuando la persona más necesita poder
  // solicitarlo de nuevo (ver renewalRequiredView, que abre esta misma
  // hoja con "Renovar suscripción").
  const expired = !!(account && account.subscriptionExpired);
  const order = ['gratis', 'premium', 'premium_plus'];
  let html = `<div class="sheet-handle"></div><h3>Elige tu plan</h3>
    <p>Los planes de paga se activan por transferencia bancaria (México) o PayPal: eliges cuántos meses quieres pagar y tocas el botón que abre WhatsApp — en cuanto se confirme el pago ahí, activamos tu plan por ese tiempo.</p>
    <div class="plan-options">`;
  order.forEach((id) => {
    const p = planInfo(id);
    const isCurrent = currentPlanId === id && !expired;
  ...
  // Cuarta opción: código promocional (15 días gratis de Premium Plus,
  // limitado a un número fijo de personas — ver public.promo_codes/
  // promo_redemptions y la función redeem_promo_code() en
  // supabase/schema.sql, y redeemPromoCode()/wirePromoCard() en app.js).
  // No es un plan más de la lista de arriba porque no es algo que se
  // "elija" entre varios — es una acción aparte (canjear), con su propio
  // campo de texto en vez de un botón de elegir.
  html += `<div class="plan-option promo-option">
    <div class="plan-option-head"><b>¿Tienes un código?</b></div>
    <p>Canjea un código promocional y obtén 15 días gratis de Premium Plus.</p>
    <div class="promo-row">
      <input type="text" id="promoCodeInput" placeholder="Código promocional" autocapitalize="characters" autocomplete="off" />
      <button type="button" class="btn btn-outline" id="promoApplyBtn">Aplicar</button>
    </div>
    <p class="auth-error" id="promoError" style="display:none;"></p>
  </div>`;
  html += `</div>`;
  return html;
}
...
export function paymentRequestSheet(planId, account) {
  ...
  // Flujo: elegir meses → elegir método → ver el total → "Pagar vía
  // WhatsApp" como único botón final. Ya no se pide el celular — es el
  // usuario quien abre la conversación de WhatsApp, así que no hace falta
  // que Memoreo lo llame. El botón queda deshabilitado hasta que se elija
  // duración y método (ver validate()/updateWhatsappLink() en
  // openPaymentRequest, app.js); su liga se arma con WHATSAPP_NUMBER (ver
  // store.js) y un mensaje ya escrito con el plan, los meses y el método.
  // Si no hay número configurado, WHATSAPP_NUMBER queda vacío y el botón
  // no se dibuja — sin él no hay forma de completar la solicitud.
  if (!WHATSAPP_NUMBER) {
    return `<div class="sheet-handle"></div><h3>${icon('cash')} Solicitar ${esc(p.label.replace('Plan ', ''))}</h3>
      <p>Todavía no está configurado el número de WhatsApp para recibir solicitudes de pago. Vuelve más tarde.</p>`;
  }
  return `<div class="sheet-handle"></div><h3>${icon('cash')} Solicitar ${esc(p.label.replace('Plan ', ''))}</h3>
    <p>Elige cuántos meses quieres pagar y cómo. Al final se abre WhatsApp para que nos escribas y te pasemos los datos bancarios o la liga de PayPal — en cuanto confirmemos el pago, activamos tu plan por ese tiempo.</p>
    ...
    <button type="button" class="btn btn-primary" id="prWhatsappBtn" data-plan="${esc(planId)}" data-monthly="${p.monthlyPrice}" data-number="${WHATSAPP_NUMBER}" disabled style="display:flex;align-items:center;justify-content:center;gap:8px;">${icon('whatsapp')} Pagar vía WhatsApp</button>`;
}
...
export function addView({ draft, editing, plan }) {
  ...
  const namePlaceholder = mode === 'salud' ? (HEALTH_NAME_PLACEHOLDERS[draft.healthType] || HEALTH_NAME_PLACEHOLDERS.vacuna)
    : (mode === 'doc' && draft.category === 'hogar') ? 'Ej. Recibo de luz, contrato de renta, garantía'
    : (mode === 'doc' && draft.category === 'estudios') ? 'Ej. Comprobante de pago, constancia de estudios'
    : (namePlaceholders[mode] || 'Ej. INE, Pasaporte, Garantía de lavadora');
  ...
  if (ci && ci.id === 'hogar') {
    // Hogar es la única categoría con tres tipos de registro — ver la nota
    // junto a formMode() en store.js. "documento" (Documentos) y "pago"
    // (Servicios) resuelven ambos a mode !== 'activity', así que el chip
    // seleccionado se decide comparando contra `mode` (no contra el kind
    // crudo, que para Servicios guardados antes de que existiera esta
    // distinción de tres puede seguir siendo el valor histórico 'doc').
    html += `<div class="field"><label>Tipo de registro</label><div class="chip-row" id="kindChips">
      <button type="button" class="chip ${mode === 'doc' ? 'selected' : ''}" data-set-kind="documento">${icon('doc')} Documentos</button>
      <button type="button" class="chip ${mode === 'pago' ? 'selected' : ''}" data-set-kind="pago">${icon('repeat')} Servicios</button>
      <button type="button" class="chip ${mode === 'activity' ? 'selected' : ''}" data-set-kind="activity">${icon('wrench')} Mantenimiento</button>
    </div></div>`;
  } else if (ci && ci.activityCapable) {
    html += `<div class="field"><label>Tipo de registro</label><div class="chip-row" id="kindChips">
      <button type="button" class="chip ${mode !== 'activity' ? 'selected' : ''}" data-set-kind="doc">${icon('doc')} ${docKindLabelSingular()}</button>
      <button type="button" class="chip ${mode === 'activity' ? 'selected' : ''}" data-set-kind="activity">${icon('wrench')} Mantenimiento</button>
    </div></div>`;
  }
  ...
  if (mode === 'salud') {
    ...
    // Recordatorio de que la foto/PDF se adjunta arriba, en #uploadZone —
    // útil sobre todo aquí (Documentos de Hogar y Estudios), donde lo más
    // común es que la persona quiera guardar el comprobante ya pagado, no
    // solo la fecha.
    if (draft.category === 'hogar') {
      html += `<p class="preset-hint">Adjunta arriba la foto o el PDF del recibo, contrato o garantía — con fecha de vencimiento o sin ella, Memoreo lo guarda igual.</p>`;
    } else if (draft.category === 'estudios') {
      html += `<p class="preset-hint">Adjunta arriba la foto o el PDF del comprobante de pago — con fecha de vencimiento o sin ella, Memoreo lo guarda igual.</p>`;
    }
    html += reminderBlock(draft, mode);
  }
...
export function detailView(doc, { plan } = {}) {
  ...
  // (frecuencia, próximo pago, monto) — ver formMode()/docInfo() en
  // store.js. "Documentos" dentro de Hogar (kind === 'documento') NO entra
  // aquí — es un documento normal con vencimiento, igual que cualquier
  // otra categoría sin tipo especial.
  const isPago = doc.category === 'pagos' || (doc.category === 'hogar' && !activity && doc.kind !== 'documento');
  ...
  if (ci.activityCapable) {
    const tipoLabel = activity ? 'Mantenimiento' : (doc.category === 'hogar' ? (doc.kind === 'documento' ? 'Documento' : 'Servicio') : docKindLabelSingular());
  ...
  // "Marcar como atendido" (ver markAttended()/unmarkAttended() en
  // app.js, isHandled()/dueDateFor() en store.js): solo aparece cuando de
  // verdad aplica — o está por vencer/pagarse pronto ahora mismo (mismo
  // criterio que "Vence pronto" en Inicio), o ya se había marcado como
  // atendido antes (para poder deshacerlo). Se acaba solo, sin que nadie
  // tenga que desmarcarlo: en cuanto pase este vencimiento y la fecha
  // avance al siguiente ciclo (o se edite el vencimiento de un
  // documento), deja de contar como atendido.
  const handled = isHandled(doc);
  if (handled) {
    html += `<div class="attend-banner attended"><span>${icon('check')}</span><div><b>Marcado como atendido.</b> Vuelve a aparecer en "Vence pronto" hasta el siguiente vencimiento.</div>
      <button type="button" class="link" data-unattend="${doc.id}">Deshacer</button></div>`;
  } else if (['crit', 'warn'].includes(info.urgency)) {
    html += `<div class="attend-banner"><span>${icon('bell')}</span><div><b>¿Ya lo atendiste?</b> Márcalo y deja de aparecer en "Vence pronto" hasta el siguiente vencimiento.</div>
      <button type="button" class="link" data-attend="${doc.id}">Marcar como atendido</button></div>`;
  }
...
// Fecha en que vence el plan de paga de esa cuenta (plan_expires_at, ver
// admin-metrics.mjs) — en rojo si ya pasó (mismo caso que
// account.subscriptionExpired en auth.js: la persona ya está viendo la
// pantalla de "Tu suscripción venció" en su propia app), o "—" para Plan
// Gratis / sin fecha.
function adminExpiryLabel(u) {
  if (!u.planExpiresAt || (u.plan || 'gratis') === 'gratis') return '—';
  const expired = new Date(u.planExpiresAt).getTime() <= Date.now();
  const label = fmtDate(new Date(u.planExpiresAt));
  return expired
    ? `<span style="color:var(--critical);font-weight:600;">${esc(label)}</span>`
    : esc(label);
}
function adminUsersTable(users, totalCount) {
  ...
  const rows = users.map((u) => `
    <tr>
      ...
      <td class="mono">${adminExpiryLabel(u)}</td>
  ...
  const cards = users.map((u) => `
    <div class="admin-user-card">
      ...
      ${(u.plan || 'gratis') !== 'gratis' ? `<div class="meta" style="border-top:none;padding-top:2px;"><span>Vence ${adminExpiryLabel(u)}</span></div>` : ''}
    </div>`).join('');
  return `
    <div class="admin-table-card"><div class="admin-table-scroll"><table class="admin-table">
      <thead><tr><th>Nombre</th><th>Correo</th><th>Plan</th><th>Vence</th><th>Registrado</th><th>Última actividad</th></tr></thead>
      <tbody>${rows}</tbody>
    </table></div></div>
...
// pantalla y requestManualPayment en auth.js) — la persona escribe por
// WhatsApp y, en cuanto el administrador confirma que ya pagó, activa la
// solicitud (o la cancela, si nunca se completó el pago). Ver
...
function adminPaymentRequestsSection(payments, loading, error) {
  ...
  if (loading && !list.length) {
    ...
    const rows = list.map((r) => {
      ...
      const deleteBtn = `<button class="btn btn-danger" style="padding:8px 12px;font-size:0.78rem;" data-delete-request="${esc(r.id)}" title="Eliminar de la lista">${icon('close')}</button>`;
      const actions = isPending
        ? `<div style="display:flex;gap:6px;flex-wrap:wrap;"><button class="btn btn-primary" style="padding:8px 12px;font-size:0.78rem;" data-activate-request="${esc(r.id)}">Activar</button><button class="btn btn-ghost" style="padding:8px 12px;font-size:0.78rem;" data-cancel-request="${esc(r.id)}">Cancelar</button>${deleteBtn}</div>`
        : `<div style="display:flex;gap:6px;flex-wrap:wrap;">${isActive ? `<button class="btn btn-outline" style="padding:8px 12px;font-size:0.78rem;" data-revert-request="${esc(r.id)}">Revertir</button>` : ''}${deleteBtn}</div>`;
      return `
      <tr>
    ...
    const cards = list.map((r) => {
      ...
      const deleteBtnCard = `<button class="btn btn-danger" style="width:100%;padding:9px;font-size:0.82rem;" data-delete-request="${esc(r.id)}">Eliminar de la lista</button>`;
      let actions;
      if (isPending) {
        actions = `<div style="display:flex;gap:8px;margin-top:10px;"><button class="btn btn-primary" style="flex:1;padding:9px;font-size:0.82rem;" data-activate-request="${esc(r.id)}">Activar</button><button class="btn btn-ghost" style="flex:1;padding:9px;font-size:0.82rem;" data-cancel-request="${esc(r.id)}">Cancelar</button></div><div style="margin-top:8px;">${deleteBtnCard}</div>`;
      } else if (isActive) {
        actions = `<div style="margin-top:10px;"><button class="btn btn-outline" style="width:100%;padding:9px;font-size:0.82rem;" data-revert-request="${esc(r.id)}">Revertir activación</button></div><div style="margin-top:8px;">${deleteBtnCard}</div>`;
      } else {
        actions = `<div style="margin-top:10px;">${deleteBtnCard}</div>`;
      }
      return `
      <div class="admin-user-card">
        <div class="top">
  ...
  return `
    <div class="admin-page-head" style="margin-top:32px;">
      ...
      <div class="sub">Transferencia bancaria o PayPal, activadas a mano — la persona te escribe por WhatsApp, confirma el pago y actívala.</div></div>
    </div>
    ${body}`;
}

// Hoja que se abre al tocar "Activar" en una solicitud (nueva o revertida,
// ver PAYMENT_DURATIONS/statusNote en adminPaymentRequestsSection más
// arriba) — deja al administrador elegir cuántos meses activar en vez de
// usar a la fuerza los meses que la persona pidió originalmente. Viene
// preseleccionada en esos meses originales (request.months) porque, la
// mayoría de las veces, es justo lo que se va a activar tal cual — pero se
// puede cambiar antes de confirmar, por ejemplo si la persona terminó
// pagando más o menos tiempo del que había pedido. Ver openActivateSheet en
// app.js, que engancha data-duration/#adminActivateConfirm.
export function adminActivateSheet(request) {
  const chips = PAYMENT_DURATIONS.map((d) => {
    const sel = d.months === request.months;
    return `<button type="button" class="chip${sel ? ' selected' : ''}" data-duration="${d.months}">${esc(d.label)}</button>`;
  }).join('');
  const p = planInfo(request.plan);
  return `<div class="sheet-handle"></div><h3>${icon('cash')} Activar ${esc(p.label)}</h3>
    <p>${esc(request.name || request.email || 'Esta persona')} pidió ${esc(PAYMENT_DURATION_LABEL[request.months] || (request.months + ' mes(es)'))}. Confirma esa duración o elige otra antes de activar.</p>
    <div class="field"><label>¿Cuántos meses activar?</label><div class="chip-row" id="adminActivateDurations">${chips}</div></div>
    <button class="btn btn-primary" id="adminActivateConfirm" data-request-id="${esc(request.id)}">Confirmar activación</button>
