...
// Stripe, que es donde de verdad viven las facturas con su monto exacto.
// Para todos los demás casos (cuenta demo, prueba gratis sembrada sin
// pasar por Stripe, o alguien que canceló y bajó a Gratis) esta hoja arma
// un historial con los datos reales que sí tenemos en la cuenta — plan,
// tarjeta guardada, fecha de la prueba — en vez de solo decir "no hay
// nada todavía" aunque la persona sí tenga una prueba o un plan activo.
export function billingSheet(account) {
  const plan = planInfo(account && account.plan);
  const trial = trialInfo(account);
  const last4 = (account && account.paymentMethod && account.paymentMethod.last4) || null;
  const brand = (account && account.paymentMethod && account.paymentMethod.brand) || 'Tarjeta';
  const cardLine = last4 ? `${esc(brand)} terminada en ${esc(last4)}` : null;

  const rows = [];
  if (trial.active) {
    rows.push({
      icon: 'cash',
      title: `Prueba gratis · ${plan.label}`,
      when: `Vence el ${fmtDate(trial.endsAt)}`,
      amount: cardLine ? `Se cobrará ${plan.price.replace('desde ', '')} a tu ${cardLine.toLowerCase()}` : 'Sin tarjeta registrada'
    });
  } else if (plan.id !== 'gratis') {
    rows.push({
      icon: 'cash',
      title: `${plan.label} activo`,
      when: cardLine ? `Cobro a tu ${cardLine.toLowerCase()}` : 'Sin tarjeta registrada',
      amount: plan.price.replace('desde ', '')
    });
  } else if (account && account.trialUsed) {
    rows.push({
      icon: 'bell',
      title: 'Tu prueba de Premium terminó',
      when: 'Volviste al plan Gratis',
      amount: ''
    });
  }
  ...
  const intro = rows.length
    ? `<p>Este es el resumen de tu membresía. Los montos y fechas exactas de cada cobro viven en Stripe — en cuanto tengas un cliente de Stripe real, este botón te lleva directo a su portal de facturación con tus facturas completas.</p>`
