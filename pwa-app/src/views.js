...
function adminPaymentRequestsSection(payments, loading, error) {
  ...
  if (loading && !list.length) {
    ...
    const rows = list.map((r) => {
      ...
      const statusNote = r.status === 'activado' && r.activatedAt
        ? `Activado ${adminFmtDateTime(r.activatedAt)}${r.activatedBy ? ` · ${esc(r.activatedBy)}` : ''}`
        : (r.status === 'cancelado' ? 'Cancelado' : `Solicitado ${adminFmtDateTime(r.createdAt)}`);
      return `
      <tr>
    ...
    const cards = list.map((r) => {
      ...
      <div class="admin-user-card">
        ...
        ${isPending ? `<div style="display:flex;gap:8px;margin-top:10px;"><button class="btn btn-primary" style="flex:1;padding:9px;font-size:0.82rem;" data-activate-request="${esc(r.id)}">Activar</button><button class="btn btn-ghost" style="flex:1;padding:9px;font-size:0.82rem;" data-cancel-request="${esc(r.id)}">Cancelar</button></div>` : ''}
      </div>`;
    }).join('');
    body = `
      <div class="admin-table-card"><div class="admin-table-scroll"><table class="admin-table">
        <thead><tr><th>Nombre</th><th>Correo</th><th>Celular</th><th>Plan</th><th>Meses</th><th>Método</th><th>Estado</th><th>Acciones</th></tr></thead>
        <tbody>${rows}</tbody>
      </table></div></div>
      <div class="admin-card-list">${cards}</div>`;
  }
