...
// Correo de bienvenida al registrarse — "fire and forget": se llama sin
// esperar el resultado (ver register() más abajo) para que un correo que
// tarda o falla nunca retrase ni bloquee que alguien termine de crear su
// cuenta. La función de servidor (netlify/functions/send-welcome-email.mjs)
// ya se encarga de no tronar si falta configurar Resend. Cualquier error de
// red aquí también se traga, solo se avisa por consola.
function sendWelcomeEmail(name, email) {
  fetch('/api/send-welcome-email', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name, email })
  }).catch((err) => console.warn('Memoreo: no se pudo mandar el correo de bienvenida', err));
}
...
// "¿Olvidaste tu contraseña?" — a diferencia de changePassword() (que pide
// la contraseña actual), esto es para cuando ya no se acuerda de ninguna.
// Usa el propio flujo de recuperación de Supabase Auth: resetPasswordForEmail
// le manda a esa dirección un correo con una liga de un solo uso (la
// plantilla y el envío los maneja Supabase, no Resend — configurable en el
// dashboard, Authentication → Email Templates → "Reset Password"). redirectTo
// apunta de vuelta a esta misma app; Supabase agrega los tokens de sesión al
// final de esa liga como "hash" (#access_token=...&type=recovery — ver
// parseRecoveryHash() en app.js, que es quien de verdad los lee al volver).
//
// A propósito responde ok:true incluso si el correo no existe como cuenta —
// así nadie puede usar este formulario para averiguar qué correos sí están
// registrados en Memoreo.
export async function requestPasswordReset(email) {
  const cleanEmail = (email || '').trim().toLowerCase();
  if (!cleanEmail.includes('@')) return { ok: false, error: 'Escribe un correo válido.' };
  const redirectTo = window.location.origin + window.location.pathname;
  const { error } = await supabase.auth.resetPasswordForEmail(cleanEmail, { redirectTo });
  if (error) { console.warn('Memoreo: no se pudo iniciar la recuperación de contraseña', error); }
  return { ok: true };
}

// Segundo paso de la recuperación: app.js ya extrajo access_token/
// refresh_token del "hash" de la URL (ver parseRecoveryHash()) y llama aquí
// para convertirlos en una sesión de verdad — necesaria para que el
// siguiente paso (setNewPassword) pueda llamar a updateUser(). Si la liga ya
// venció o se usó antes, Supabase responde con error aquí.
export async function beginPasswordRecovery(accessToken, refreshToken) {
  const { error } = await supabase.auth.setSession({ access_token: accessToken, refresh_token: refreshToken });
  if (error) return { ok: false, error: 'Esa liga ya venció o no es válida — pide otra desde "¿Olvidaste tu contraseña?".' };
  return { ok: true };
}

// Último paso: ya con la sesión de recuperación activa (ver
// beginPasswordRecovery arriba), de verdad cambia la contraseña. A
// diferencia de changePassword() no hace falta la contraseña vieja — la liga
// de un solo uso ya demostró que es dueño del correo. Deja a la persona con
// la sesión iniciada de una vez (mismo formato que login()/register()), para
// no pedirle que además inicie sesión a mano con la contraseña que acaba de
// escoger.
export async function setNewPassword(newPassword) {
  ...
  const account = await buildAccount(user, profile);
  return { ok: true, account };
}
