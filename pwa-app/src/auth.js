...
// plan activado a mano; maybeExpirePlan() baja la cuenta a Gratis sola en
// cuanto se cumple esa fecha. (Las funciones y campos relacionados con
// Stripe — applyStripeSubscription, trialEndsAt, trialUsed,
// stripeCustomerId — se conservan sin usar por si alguna cuenta vieja
// todavía los trae, pero ya no los llama ninguna pantalla nueva.)
...
async function buildAccount(user, profile) {
  return {
    ...
    planExpiresAt: (profile && profile.plan_expires_at) || null,
...
// Si el plan de paga se activó a mano (ver requestManualPayment/
// planExpiresAt arriba) y ya venció, baja la cuenta a Gratis sola, sin que
// nadie tenga que acordarse de hacerlo — se llama antes de armar la cuenta
// pública, tanto al recuperar la sesión (currentAccount) como al iniciar
// sesión (login), para que nunca se muestre ni se use un plan de paga ya
// vencido. Cuentas activadas por Stripe (plan_expires_at vacío) no se ven
// afectadas — esas siguen su propio ciclo con Stripe.
async function maybeExpirePlan(user, profile) {
  if (!profile || profile.plan === 'gratis' || !profile.plan_expires_at) return profile;
  if (new Date(profile.plan_expires_at).getTime() > Date.now()) return profile;
  const { data, error } = await supabase.from('profiles')
    .update({ plan: 'gratis', plan_expires_at: null })
    .eq('id', user.id)
    .select()
    .single();
  if (error) { console.warn('Memoreo: no se pudo bajar el plan vencido a Gratis', error); return profile; }
  return data;
}
...
export async function currentAccount() {
  ...
  try {
    ...
    profile = await maybeExpirePlan(session.user, profile);
...
export async function login({ email, password }) {
  ...
  profile = await maybeExpirePlan(data.user, profile);
  const account = await buildAccount(data.user, profile);
...
// applyStripeSubscription(), not this function.
...
// la app guarda la solicitud (plan, cuántos meses, método y el celular
// donde se le puede contactar) en public.payment_requests — una policy de
// RLS ("insert own") deja que cada quien inserte la suya, nada más. El
// plan NO cambia todavía: el administrador ve la solicitud en su panel, le
// manda a esa persona los datos bancarios o la liga de PayPal por fuera de
// la app, y en cuanto confirma que ya se pagó, activa la solicitud desde
// ahí — eso es lo único que de verdad sube el plan (ver
// netlify/functions/admin-payment-requests.mjs).
export async function requestManualPayment(accountId, { plan, months, paymentMethod, phone }) {
  ...
  if (cleanPhone.length < 10) return { ok: false, error: 'Escribe un número de celular válido (10 dígitos).' };
