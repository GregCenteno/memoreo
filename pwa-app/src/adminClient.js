...
// months es opcional — si no se manda (o no es una duración válida), el
// backend usa los meses que trae la solicitud original (ver VALID_MONTHS en
// admin-payment-requests.mjs). Se manda cuando el administrador elige la
// duración a mano al activar (o reactivar una solicitud revertida) — ver
// adminActivateSheet en views.js.
export async function activatePaymentRequest(creds, requestId, months) {
  return callPaymentRequests(creds, { action: 'activate', requestId, months });
}
...
// Borra el renglón de la lista para siempre (no revierte ninguna
// activación) — ver la nota completa en admin-payment-requests.mjs
// (action: 'delete').
export async function deletePaymentRequest(creds, requestId) {
  return callPaymentRequests(creds, { action: 'delete', requestId });
}
