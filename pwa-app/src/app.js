...
// El riel de navegación se puede ocultar en el celular para ganar espacio
// horizontal (ver renderTabbar/toggleRail) — se recuerda entre sesiones en
// este mismo navegador con localStorage (no es información de la cuenta,
// así que no hace falta guardarla en Supabase).
const RAIL_PREF_KEY = 'memoreo:railCollapsed';
function loadRailCollapsed() {
  try { return localStorage.getItem(RAIL_PREF_KEY) === '1'; } catch { return false; }
}
function saveRailCollapsed(v) {
  try { localStorage.setItem(RAIL_PREF_KEY, v ? '1' : '0'); } catch { /* modo privado, etc. — no pasa nada */ }
}
...
function renderTabbar() {
  appShellEl.classList.toggle('rail-collapsed', state.railCollapsed);
  if (state.route === 'welcome' || state.route === 'auth') { tabbarEl.style.display = 'none'; return; }
  tabbarEl.style.display = 'flex';
  const toggleLabel = state.railCollapsed ? 'Mostrar menú' : 'Ocultar menú';
  tabbarEl.innerHTML = `<div class="sidebar-brand">${logoMark(32)} Memoreo</div>`
    + `<button class="rail-toggle" data-action="toggle-rail" aria-label="${toggleLabel}" title="${toggleLabel}">${icon('chev')}</button>`
    + TABS.map((t) => {
