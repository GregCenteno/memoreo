...
import {
  currentAccount, register, login, logout, startDemoSession, setPlan, setAccentColor, setAvatar,
...
import { sendReminderEmail } from './emailClient.js';
...
import {
  ...
  profileView, addView, detailView, plansSheet, colorSheet, securitySheet, billingSheet, adminDashboardView
} from './views.js';

const TABS = [
  { route: 'home', icon: 'home', label: 'Inicio' },
  { route: 'search', icon: 'search', label: 'Buscar' },
  { route: 'add', icon: 'plus', label: 'Agregar', fab: true },
...
let screenEl, tabbarEl, sheetLayerEl, sheetBodyEl, toastEl, toastMsgEl, appShellEl, adminShellEl;
let toastTimer = null;
...
export async function initApp(root) {
  ...
  tabbarEl.addEventListener('click', handleClick);
...
function render() {
  ...
  switch (state.route) {
    ...
    case 'profile': screenEl.innerHTML = profileView(store.all(), state.account); break;
...
function renderTabbar() {
  if (state.route === 'welcome' || state.route === 'auth') { tabbarEl.style.display = 'none'; return; }
  tabbarEl.style.display = 'flex';
  tabbarEl.innerHTML = `<div class="sidebar-brand">${logoMark(32)} Memoreo</div>` + TABS.map((t) => {
    const active = state.route === t.route;
    if (t.fab) {
      return `<button class="tab fab-tab" data-nav="add" aria-label="${t.label}"><span class="fab">${icon('plus')}</span><span>${t.label}</span></button>`;
    }
...
async function startDemo() {
  try {
    const { account, isNew } = await startDemoSession();
    if (isNew) await initAccountDocs(account.id, true);
    state.account = account;
    await openStoreForAccount(account.id);
    applyAccentColor(effectiveAccentColor(account));
    go('home');
    toast('Sesión demo iniciada');
  } catch (err) {
    toast(err.message || 'No se pudo iniciar la sesión demo.');
  }
}
...
function postRenderAdd() {
  ...
  const labTypeChips = document.getElementById('labTypeChips');
  if (labTypeChips) {
    labTypeChips.addEventListener('click', function (e) {
      const b = e.target.closest('[data-set-lab-type]');
      if (!b) return;
      const type = b.getAttribute('data-set-lab-type');
      if (type === state.draft.labTestType) return;
      // Different lab types have different fields, so the values captured
      // for the old type wouldn't map to the new one — reset them.
      state.draft.labTestType = type;
      state.draft.labValues = {};
      render();
    });
  }
  const labFields = document.getElementById('labFields');
  if (labFields) {
    labFields.querySelectorAll('[data-lab-field]').forEach((inp) => {
      inp.addEventListener('input', () => {
        state.draft.labValues[inp.getAttribute('data-lab-field')] = inp.value;
      });
    });
  }
...
async function saveDraft() {
  ...
  try {
    if (state.editingId) {
      ...
      const msgMap = { pago: 'Pago guardado', prestamo: 'Préstamo guardado', activity: 'Registro guardado', doc: 'Documento guardado', salud: healthTypeInfo(d.healthType).savedLabel };
      toast(msgMap[mode]);
...
function confirmDelete(id) {
  ...
  const delMsg = { documento: 'Documento eliminado', registro: 'Registro eliminado', pago: 'Pago eliminado', 'préstamo': 'Préstamo eliminado' };
  openSheet(`
    <div class="sheet-handle"></div>
...
// No hay backend que hospede una URL propia por documento, así que aquí no
// se comparte "un enlace" — se comparte un resumen de texto real a través
// del share sheet nativo del sistema operativo (WhatsApp, Mensajes, correo,
// lo que sea que la persona tenga instalado). En navegadores de escritorio
// que no soportan la Web Share API, el resumen se copia al portapapeles en
// su lugar, con un mensaje que dice exactamente eso — nunca un botón que
// dice haber compartido algo sin haber hecho nada de verdad.
async function shareDoc() {
  ...
  if (navigator.share) {
    try {
      await navigator.share({ title: 'Memoreo', text });
    } catch (e) {
      // La persona canceló el share sheet — no es un error que haya que avisar.
    }
    return;
  }
...
/* ---------------- Email reminders (correo real con Resend, ver src/emailClient.js) ---------------- */
function testEmailReminder() {
  if (!state.account) return;
  const docs = store.all().filter((d) => d.expiresAt);
  if (!docs.length) { toast('Agrega un documento con fecha para probar esto'); return; }
  const target = docs.sort((a, b) => docInfo(a).days - docInfo(b).days)[0];
  const info = docInfo(target);
  const firstName = (state.account.name || '').trim().split(/\s+/)[0] || '';
  const subject = `${target.name} — ${info.label}`;
  const bodyHtml = `<p>Hola ${esc(firstName)},</p><p><b>${esc(target.name)}</b> (${esc(catInfo(target.category).label)}) ${esc(info.label).toLowerCase()}. Entra a Memoreo para ver los detalles.</p>`;
  openSheet(`
    <div class="sheet-handle"></div>
    <h3>${icon('mail')} Recordatorio por correo</h3>
    <div class="email-preview">
      <div class="email-preview-row"><span>De</span><b>Memoreo</b></div>
      <div class="email-preview-row"><span>Para</span><b>${esc(state.account.email)}</b></div>
      <div class="email-preview-row"><span>Asunto</span><b>${esc(subject)}</b></div>
      <div class="email-preview-body">${bodyHtml}</div>
    </div>
    <button class="btn btn-primary" id="sendRealEmailBtn">${icon('mail')} Enviar este correo</button>
  ...
  sendBtn.addEventListener('click', async () => {
    ...
    try {
      await sendReminderEmail({ to: state.account.email, subject, html: bodyHtml });
      toast(`Correo enviado a ${state.account.email}`);
      closeSheet();
    } catch (err) {
...
function handleClick(e) {
  ...
  if (act) {
    ...
    else if (a === 'test-email-reminder') testEmailReminder();
    else if (a === 'start-demo') startDemo();
