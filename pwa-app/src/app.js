...
import {
  ...
  fetchPaymentRequests, activatePaymentRequest, cancelPaymentRequest
} from './adminClient.js';
import {
  welcomeView, authView, homeView, categoryView, searchView, remindersView,
  profileView, addView, detailView, plansSheet, paymentRequestSheet, colorSheet, securitySheet, billingSheet, themeSheet, adminDashboardView
} from './views.js';
...
async function activatePayment(requestId) {
  if (!state.adminCreds || !requestId) return;
  try {
    await activatePaymentRequest(state.adminCreds, requestId);
    toast('Plan activado');
...
function postRenderAdmin() {
  ...
  document.querySelectorAll('[data-activate-request]').forEach((btn) => {
    btn.addEventListener('click', () => activatePayment(btn.getAttribute('data-activate-request')));
  });
