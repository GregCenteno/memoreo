...
import {
  ...
  paymentHistory, redeemPromoCode, requestPasswordReset, beginPasswordRecovery, setNewPassword
} from './auth.js';
...
import {
  ...
  adminDashboardView, adminChangePasswordSheet, renewalRequiredView, adminActivateSheet, newPasswordView
} from './views.js';
...
const state = {
  ...
  account: null,
  authError: null,
  // "¿Olvidaste tu contraseña?" — ver parseRecoveryHash/postRenderNewPassword
  // más abajo. Solo se usa mientras state.route === 'recovery'.
  recoveryError: null,
...
export async function initApp(root) {
  ...
  // "¿Olvidaste tu contraseña?": si el navegador acaba de volver de la liga
  // de recuperación que mandó Supabase por correo, sus tokens vienen en el
  // "hash" de la URL (ver parseRecoveryHash más abajo) — hay que revisar
  // esto ANTES que la restauración de sesión normal de aquí abajo, porque
  // detectSessionInUrl está apagado a propósito en supabaseClient.js (ver la
  // nota ahí) y por eso Supabase nunca los procesa solo. Si hay un hash de
  // recuperación válido, se salta por completo el flujo normal de abajo (la
  // cuenta que hubiera logueada antes en este navegador no importa, porque
  // esta liga es de otra persona o de la misma persona empezando de cero) y
  // se manda derecho a la pantalla de nueva contraseña.
  const recovery = parseRecoveryHash();
  if (recovery) {
    window.history.replaceState(null, '', window.location.pathname + window.location.search);
    const result = await beginPasswordRecovery(recovery.accessToken, recovery.refreshToken);
    if (result.ok) {
    ...
    toast(result.error);
    // Sigue con el flujo normal de abajo en vez de dejar a la persona
    // atorada en una pantalla en blanco.
  }
...
// Lee del "hash" de la URL (lo que va después de #) los tokens que Supabase
// agrega al final de la liga de recuperación de contraseña que manda por
// correo (ver requestPasswordReset en src/auth.js) — algo como
// "#access_token=...&refresh_token=...&type=recovery". Devuelve null si no
// es esa liga (por ejemplo, si la persona nomás abrió la app normal). Pura
// función de lectura, no toca la sesión ni la URL — eso lo hace initApp.
...
function render() {
  ...
  switch (state.route) {
    ...
    case 'recovery': screenEl.innerHTML = newPasswordView({ error: state.recoveryError }); break;
  ...
  if (state.route === 'recovery') postRenderNewPassword();
...
function renderTabbar() {
  const hide = state.route === 'welcome' || state.route === 'auth' || state.route === 'recovery' || (state.account && state.account.subscriptionExpired);
...
// Hoja de "¿Olvidaste tu contraseña?" — se abre desde el enlace en
// authView (solo en modo "Iniciar sesión", ver postRenderAuth arriba).
// Solo pide el correo; requestPasswordReset() en src/auth.js siempre
// responde ok:true (aunque el correo no exista, a propósito, para no
// revelar qué correos sí están registrados), así que aquí no hay un
// "camino de error" real que mostrar — nomás confirma que se mandó el
// enlace, si es que había algo que mandar.
function openForgotPassword() {
  openSheet(`
    <div class="sheet-handle"></div>
    <h3>${icon('mail')} ¿Olvidaste tu contraseña?</h3>
    <p>Escribe el correo de tu cuenta y te mandamos una liga para escoger una contraseña nueva.</p>
    <div class="field">
      <label for="forgotEmail">Correo</label>
      <input type="email" id="forgotEmail" placeholder="tucorreo@ejemplo.com" autocomplete="email">
    </div>
    <p class="auth-error" id="forgotError" style="display:none;"></p>
    <button type="button" class="btn btn-primary" id="forgotSubmitBtn">Mandar liga</button>
  `);
  ...
  submitBtn.addEventListener('click', async () => {
    ...
    const result = await requestPasswordReset(input.value);
    submitBtn.disabled = false;
    submitBtn.textContent = 'Mandar liga';
    if (!result.ok) {
    ...
    closeSheet();
    toast('Te mandamos una liga — revisa tu correo');
  });
}

// Pantalla de "Nueva contraseña" (state.route === 'recovery', ver
// parseRecoveryHash/initApp más arriba) — a propósito no tiene botón de
// "atrás" en newPasswordView (ver views.js), así que el único camino de
// salida es #recoveryCancelBtn, que cierra sesión de verdad antes de volver
// a la pantalla de bienvenida (en vez de nomás cambiar de pantalla) — así no
// se queda colgada una sesión de recuperación iniciada a medias si alguien
// abandona esto en una computadora compartida.
function postRenderNewPassword() {
  ...
  function validate() {
    submitBtn.disabled = !(p1.value.length >= 6 && p1.value === p2.value);
  }
  p1.addEventListener('input', validate);
  p2.addEventListener('input', validate);
  validate();
  wirePasswordToggle(p1, document.getElementById('newPasswordToggle'));

  submitBtn.addEventListener('click', async () => {
    submitBtn.disabled = true;
    const result = await setNewPassword(p1.value);
    if (!result.ok) {
      state.recoveryError = result.error;
      render();
      return;
    }
    state.account = result.account;
    ...
    trackAccount(result.account);
    go('home');
    toast('Contraseña actualizada — ya iniciaste sesión');
  });
