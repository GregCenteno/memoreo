...
function openPaymentRequest(plan) {
  ...
  const phoneInput = document.getElementById('prPhone');
  ...
  if (!submitBtn) return;
  const monthly = +submitBtn.getAttribute('data-monthly') || 0;
  ...
  function validate() {
    const phoneOk = phoneInput.value.trim().length >= 10;
    submitBtn.disabled = !(selectedMonths && selectedMethod && phoneOk);
  }
  // El botón de WhatsApp (si hay número configurado, ver WHATSAPP_NUMBER en
  // store.js) no espera a que se elija duración/método como el botón de
  // arriba — está disponible desde que se abrió esta hoja, con el plan
  // nada más. Cada vez que sí se elige duración o método, el mensaje
  // prellenado se actualiza para incluirlos.
  function updateWhatsappLink() {
    if (!whatsappBtn) return;
    whatsappBtn.href = `https://wa.me/${whatsappBtn.getAttribute('data-number') || ''}?text=${encodeURIComponent(whatsappPayMessage(plan, selectedMonths, selectedMethod))}`;
  }
  if (whatsappBtn && !whatsappBtn.getAttribute('data-number')) {
    // La liga ya viene armada desde paymentRequestSheet (con el número
    // real) — aquí solo se guarda ese número aparte para poder reconstruir
    // la liga cada vez que cambie duración/método, sin tener que volver a
    // importar WHATSAPP_NUMBER en app.js.
    const existingUrl = new URL(whatsappBtn.href);
    whatsappBtn.setAttribute('data-number', existingUrl.pathname.replace('/', ''));
  }
  ...
  submitBtn.addEventListener('click', async () => {
    ...
    const result = await requestManualPayment(state.account.id, {
      plan, months: selectedMonths, paymentMethod: selectedMethod, phone: phoneInput.value.trim()
    });
    if (!result.ok) {
...
function openSubscription() {
  if (!state.account) return;
  openSheet(subscriptionSheet(state.account));
}
